...
def clean_response(response: str) -> str:
    ...
    try:
        ...
        # Aggressively remove any source citations or headers that might have slipped through
        # e.g., "Source: Header...", "Sources: ...", "[From file...]"
        response_clean = re.sub(r'(?i)^(Source|Sources|Source:).*$', '', response_clean, flags=re.MULTILINE)
        response_clean = re.sub(r'^\[From.*?\]:?.*$', '', response_clean, flags=re.MULTILINE)
        
        # Remove common introductory phrases to ensure direct answers
        response_clean = re.sub(r'(?i)^(Based on|According to) (the )?(provided )?(documents|context|information)[,:;]?\s*', '', response_clean)
...
def format_context_with_sources(context: list) -> str:
    ...
    for i, doc in enumerate(context):
        ...
        # Format chunk (Source attribution removed as per user request)
...
async def generate_response(
    ...
    try:
        ...
        # Sources section logic removed as per request
