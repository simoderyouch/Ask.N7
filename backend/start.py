...
Handles:
1. Docker services startup (PostgreSQL, Redis, MinIO, Qdrant)
...
def start_docker_services(rebuild=False):
    ...
    for i in range(30): 
        result = run_command(
            ["docker", "ps", "--filter", "name=n7_postgres", "--format", "{{.Status}}"],
            check=False,
            capture_output=True
        )
    ...
    for i in range(60):  
        result = run_command(
            ["docker", "exec", "n7_postgres", "pg_isready", "-U", "postgres", "-d", "hcp"],
            check=False,
            capture_output=True
        )
...
def start_application():
    ...
    print("\n" + "="*60)
    print("STEP 2: Starting Application")
    print("="*60)
...
def main():
    ...
    print("="*60)
    print("\nThis script will:")
    print("  1. Start all Docker services (PostgreSQL, Redis, MinIO, Qdrant)")
    if not args.no_app:
    ...
    try:
        ...
        # Step 2: Start the application (unless --no-app flag is set)
        if not args.no_app:
            start_application()
        else:
            print("\n" + "="*60)
